...
int main_BFS()
{
	...
	error = BFS_Initialization("ASRL14::INSTR", &MyBFS_ID);//choose the com port, can be ASRLXXX::INSTR (where XXX=port number)
	...
	do {//loop until exit
		...
		if (answer == "get flow")
		{
			//get the volumetric flow (!!!! NEED TO measure density at least one time before using this function, density is used to measure the flux !!!!)
		...
		if (answer == "set filter")
		{
			...
			//ask the filter value, and set it
